...
const getItemsFromWarehouse = async (warehouseName: string) => {
    const response = await axios.get(`http://localhost:8000/v1/warehouses/${warehouseName}/items?limit=10`);

    return response.data.items;
}

const getWarehouses = async () => {
    const response = await axios.get(`http://localhost:8000/v1/warehouses`);
